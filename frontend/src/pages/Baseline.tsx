    ...
    const fetchChildrenAndScores = async () => {
      ...
      try {
        ...
        if (dataChildren.status === "success") {
          const sorted = [...dataChildren.data].sort((a, b) => a.full_name.localeCompare(b.full_name));
  ...
  return (
    <div className="min-h-screen bg-[#F4F1E3] px-4 py-6 flex flex-col items-center">
      ...
      <div className="w-full max-w-3xl bg-white p-6 rounded-lg shadow space-y-4">
        ...
        {/* Table Header */}
        {children.length > 0 && (
          <div className="hidden sm:grid grid-cols-[1fr_2fr_2fr_2fr_2fr] bg-[#7A3D1A] text-white px-4 py-2 rounded-md text-sm font-semibold mb-3">
            <div>Sl No</div>
            <div>Name</div>
            <div>Class</div>
        ...
        {isLoading ? (
          ...
          children.map((child, index) => (
            <div
              key={child.id}
              className="grid sm:grid-cols-[1fr_2fr_2fr_2fr_2fr] grid-cols-1 gap-y-2 gap-x-4 items-center p-4 bg-gray-50 border border-gray-200 rounded-lg shadow-sm text-sm mb-3"
            >
              <div className="sm:col-span-1">
                <span className="sm:hidden font-semibold text-gray-600">Sl No: </span>
                {index + 1}
              </div>
              <div>
                <span className="sm:hidden font-semibold text-gray-600">Name: </span>
                {child.full_name}
              </div>
              <div>
                <span className="sm:hidden font-semibold text-gray-600">Class: </span>
                {child.child_class}
              </div>

              {/* Reading Level Dropdown */}
              <div>
                <span className="sm:hidden font-semibold text-gray-600">Reading: </span>
                <select
              ...
              <div>
                <span className="sm:hidden font-semibold text-gray-600">Speaking: </span>
                <select
